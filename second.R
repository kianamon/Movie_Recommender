...
df_kianatrain$Genres1 <- gsub('\\s+', '', df_kianatrain$Genres1)
df_kianatrain$Genres2 <- gsub('\\s+', '', df_kianatrain$Genres2)
df_kianatrain$Genres3 <- gsub('\\s+', '', df_kianatrain$Genres3)
df_kianatrain$Genres4 <- gsub('\\s+', '', df_kianatrain$Genres4)
...
saveplot <- ggplot(my_data,aes(predicted, observed)) + 
...
sparse_matrix <- sparse.model.matrix(Likability ~ ., data = df_kianatrain1)[,-1]
names(sparse_matrix)
ImpMeasure<-data.frame(varImp(xgb_model)$importance)
ImpMeasure$Vars<-row.names(ImpMeasure)
ImpMeasure[order(-ImpMeasure$Overall),][1:50,]
#####################################################################################
#submission
